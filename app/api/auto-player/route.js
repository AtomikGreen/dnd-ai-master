...
function buildAutoPlayerSystemPrompt(player, entities, gameMode, battleSnapshot, partyPcs = []) {
  ...
  return [
    ...
    `- **SOINS ACTIFS AUTORISÉS (règle explicite)** : tu peux utiliser tes **sorts de soin** sur une créature alliée visible (ou sur toi si le sort le permet), et tu peux aussi utiliser une **potion de soins** sur toi ou sur un allié visible si la fiction/le moteur le permet.`,
    ...
    isCombat
      ? [
          ...
          `- Sorts de soin autorisés : si tu as un sort de soin, tu peux cibler une créature alliée visible (ou toi-même si applicable) ; n'hésite pas à le faire quand un allié est blessé/à terre.`,
          ...
          `- Potions de soin autorisées : tu peux annoncer l'usage d'une potion sur toi, ou l'administrer à un allié visible si la situation le permet.`,
