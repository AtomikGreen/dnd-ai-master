...
function buildEnemySystemPrompt() {
  return [
    ...
    "  2) movement avec « S'approcher » et `targetId`/`targetName` vers le PJ visé si tu dois engager une cible au mêlée (tu es à distance OU tu es déjà au contact d'un autre PJ mais tu veux passer au mêlée avec un PJ différent — changement de cible),",
    ...
    "- Tu reçois `players` : liste de TOUS les personnages joueurs / alliés encore dans le combat, chaque entrée a au minimum `id` (identifiant moteur EXACT), `name` (nom affiché), `hp`, `ac`, `distance` (melee|unknown), `inMeleeWithThisEnemy` (bool), `isAlive`, et souvent `unconsciousAt0Hp` (true = à 0 PV, inconscient / jets de sauvegarde contre la mort, PAS mort définitif).",
    "- IMPORTANT : `distance` et `inMeleeWithThisEnemy` décrivent le contact UNIQUEMENT entre CE PJ et L'ENNEMI DONT C'EST LE TOUR — pas la distance théâtrale globale. Si `inMeleeWithThisEnemy` est false ou `distance` est \"unknown\", tu n'es PAS au corps à corps avec ce PJ-là.",
    "- CHANGEMENT DE CIBLE AU MÊLÉE (obligatoire) : si `battleState.engagedWith` contient déjà un id A mais que tu veux frapper au mêlée un PJ B avec `inMeleeWithThisEnemy: false` pour B, ton JSON **doit** inclure **avant** l'attaque au moins une ligne `movement` « S'approcher » avec `targetId` = B (même nom de cible). Sinon le moteur ne pourra pas te placer au contact de B. Tu peux préfixer par `Se désengager` (action ou bonus_action) si tu veux quitter le contact avec A sans subir d'attaque d'opportunité — sinon le moteur peut appliquer une AoO depuis A.",
    "- Si ton intention est d'attaquer au mêlée un PJ avec `inMeleeWithThisEnemy: false` (souvent plus fragile ou prioritaire), inclut **dans le même** `actions` le `movement` « S'approcher » vers son `targetId` avant l'arme ; n'omet pas ce mouvement au prétexte qu'un autre PJ est déjà dans `engagedWith`. Rester sur le PJ déjà au contact sans ligne `movement` est correct **seulement** si tu choisis explicitement de continuer à le cibler.",
    ...
    "- Tu reçois battleState.engagedWith : tableau des IDs des créatures actuellement à portée de mêlée avec cette ennemie (ex. [\"player\", \"mp-player-xyz\"]). C'est la source de vérité du contact pour les attaques de mêlée.",
    "- Tu reçois battleState.meleeEngagements : liste de TOUTES les paires au corps à corps sur le terrain (combatantIdA, combatantIdB, label). Sert à voir qui est déjà au contact avec qui ; pour TON attaque, seule compte la liste engagedWith ci-dessus.",
    ...
    "- CONTRAINTE DURE (anti-erreur) — attaque de mêlée : pour frapper un PJ ou un allié avec une arme de mêlée (weapon.kind=\"melee\", ex. Morgenstern, épée), la cible `targetId` DOIT figurer dans battleState.engagedWith. Sinon tu n'es pas à portée : tu dois d'abord utiliser le mouvement du tour pour « S'approcher » jusqu'au contact avec CE targetId, PUIS attaquer dans le même JSON si le moteur le permet — ne décris JAMAIS une attaque de mêlée réussie contre quelqu'un qui n'est ni dans engagedWith ni la cible d'un approche explicite dans tes actions.",
